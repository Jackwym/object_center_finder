...
def calculate_pixel_error(p, px, py, dist, bias_array, width, height):
    ...
    for x in range(max(0, px - dist), min(int(width), px + dist)):
        for y in range (max(0, py - dist), min(int(height), py + dist)):
            ...
            #     # print(x)
            #     sum_error += 300 * individual_error
...
def make_error_map(image, dist, bias_array):
    ...
    for x in range(0, image.width - 20):
        for y in range (0, image.height - 20):
            if (math.sqrt(x ** 2 + y ** 2) > dist): continue
            max_error += 765 * DISTANCE_BIAS * (x ** 2 + y ** 2) / dist
    ...
    for x in range(0, image.width - 1):
        ...
        for y in range (0, image.height - 1):
            ...
            pixels[x, y] = (pixel_error, pixel_error, pixel_error)
    pixels[0, 0] = (255, 0, 0)
